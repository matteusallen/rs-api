...
import { STRIPE_ACCOUNT_TYPE, ADD_ON_PRODUCT_X_REF_TYPE_ID, RESERVATION_X_REF_TYPE_ID } from 'Constants';
import { em } from '../../../utils/processReport';
import { getRvs, getRVProduct, getRVData } from './accountingReportHelper';
...
//This is to pull reports for stripe standard account type
//because they have no payouts
async function accountingReport(input: AccountingReportInputType, request?: any): Promise<any> {
  try {
    const { User, AddOnProduct, Reservation, Order, UserVenue, OrderHistoryPayments, OrderHistory, RVProduct } = this.sequelize.models;
    const { start, end, eventIds, venueId, venueTimeZone, userId } = input;

    let whereClause,
      initialClause = { success: true, stripeAccountType: STRIPE_ACCOUNT_TYPE.standard, cardPayment: true };
    ...
    if (eventIds && eventIds.length) {
      ...
      const adminIds = venueAdmins.map(venueAdmin => venueAdmin.userId);
      whereClause = {
        ...initialClause,
    ...
    const payments = await this.findAll({
      where: whereClause,
      attributes: ['id', 'adminId', 'amount', 'createdAt', 'notes', 'ssChargeId', 'ssRefundId', 'cardBrand', 'stripeFee', 'serviceFee'],
      include: [
        {
          association: 'order',
          ...
          include: [
            ...
            },
            {
              ...
              attributes: ['xRefTypeId', 'xProductId', 'quantity', 'price']
            }
          ]
        }
      ],
    ...
    const columns = [
      ...
      { header: 'Lot Switch', key: 'hasMultipleCharges', width: 0 }
    ];

    const dataRows = [];
    // aggregates of all payments
    ...
    for (let i = 0; i < payments.length; i++) {
      ...
      if (payment.ssRefundId) {
        ...
        const stripeFeeRounded = payment.stripeFee ? Math.round(payment.stripeFee * 100) / 100 : 0;
        const stripeFee = Number(stripeFeeRounded.toFixed(2));
        const totalFee = stripeFee + Number(payment.serviceFee.toFixed(2));
        paymentTransactionTotal = paymentGrossAmount - Number(totalFee.toFixed(2));
      ...
      rowData.transactionTotal = paymentTransactionTotal;
      rowData.calculatedSubtotal = paymentTransactionTotal;
      rowData.difference = 0;

      dataRows.push(rowData);

      // calculate payment aggregate amounts
